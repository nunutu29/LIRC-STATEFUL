...
static int stateful_send(int fd, char *message, char *arguments);
static int stateful_get(int fd, char *message, char *arguments);
...
static const struct protocol_directive directives[] = {
	...
	{ "SIMULATE",	      	simulate	       	},
	{ "STATEFUL_SEND",      stateful_send  	    },
	{ "STATEFUL_GET",  		stateful_get		},
	{ NULL,		      		NULL	       		}
	/*
	 * {"DEBUG",debug},
...
int send_client_output(int fd, char* message,const char* format_str, ...)
{
	log_debug("Sending client output");
...
static int stateful_send(int fd, char *message, char *arguments)
{
	...
	log_debug("Stateful send, msg: %s, args: %s", message, arguments);
	if (curr_driver->send_mode == 0)
		return send_error(fd, message, "hardware does not support sending\n");

	name = strtok_r(arguments, WHITE_SPACE, &commands);
	if(name == NULL)
		return send_error(fd, message, "undefined remote name\n");

	remote = get_ir_remote(remotes, name);
	if(remote == NULL)
		return send_error(fd, message, "unknown remote: \"%s\"\n", name);

	code = get_stateful_ir_ncode(remote, commands, WHITE_SPACE);
	if(code == NULL)
		return send_error(fd, message, "unknown command");
	...
	if(!send_ir_ncode(remote, code, 1)){
		free_stateful_ir_ncode(code);
		return send_error(fd, message, "transmission failed\n");
	}

	if(remote->last_sent_code != NULL){
		// at this point the send succeed
		// delete the last sent code
		free_stateful_ir_ncode(remote->last_sent_code);
	}
	// save code as last_sent_code
	remote->last_sent_code = code;

	gettimeofday(&remote->last_send, NULL);

	remote->repeat_countdown = max(remote->repeat_countdown, 0);

	if (remote->repeat_countdown > 0) {
		log_debug("start repeating");
...
static int stateful_get(int fd, char *message, char *arguments){
	...
	remote_name = strtok(arguments, WHITE_SPACE);
	if(remote_name == NULL)
		return send_error(fd, message, "undefined remote name\n");

	remote = get_ir_remote(remotes, remote_name);
	if(remote == NULL)
		return send_error(fd, message, "unknown remote: \"%s\"\n", remote_name);

	name = get_stateful_last_name(remote);
	...
	for (n=0; p_str[n]; p_str[n] == ',' ? n++ : *p_str++);
	...
	token = strtok_r(str, SPLITTER_CHAR_2, &p_str);
	while(token != NULL){
		if(strlen(token) > 0){
			len = snprintf(buffer, PACKET_SIZE, "%s\n", token);
			if (len >= PACKET_SIZE + 1)
				len = sprintf(buffer, "code_too_long\n");
			if (write_socket(fd, buffer, len) < len)
				return 0;
		}
		token = strtok_r(NULL, SPLITTER_CHAR_2, &p_str);
	}
