...
static const char* const help =
	...
	"    irsend [options] STATEFUL_SEND remote [code...]\n"
...
int main(int argc, char** argv)
{
	...
	if (strcasecmp(directive, "simulate") == 0) {
		...
		//INIT Johnny Code
		//####################################################################################
		//Added new code only inside this section and of course the else clause after this
		if (strcasecmp(directive, "STATEFUL_SEND") == 0
			|| strcasecmp(directive, "STATEFUL_GET") == 0)
		{
			char buffer[PACKET_SIZE + 1];
			int len;
			size_t buffer_limit = PACKET_SIZE;

			while (optind < argc) {
				len = snprintf(&buffer[PACKET_SIZE - buffer_limit], buffer_limit, "%s ", argv[optind++]);
				if(len > 0 && (unsigned int)len < buffer_limit){
					buffer_limit -= len;
				} else {
					close(fd);

					fprintf(stderr, "%s: input too long.\n"
									"Try to edit your configuration file.\n"
									"Command \"%s %s [commands]\" should be %d chars long.\n",
							prog, directive, remote, PACKET_SIZE);

					exit(EXIT_FAILURE);
				}
			}
			r = lirc_command_init(&ctx, "%s %s %s\n", directive, remote, buffer);
			if (r != 0) {
				close(fd);
				exit(EXIT_FAILURE);
			}
