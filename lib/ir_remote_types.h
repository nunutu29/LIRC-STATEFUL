...
struct ir_remote {
	...
	/* Added by Johnny */
	const char *checksum_alg;			/** Name of the checksum algorithm. Used only for stateful ir codes */
	struct ir_ncode* last_sent_code;	/** Last sent code: need it to remember the last state of the remote.
  											I could use the "last_send" field, but somehow it had an undefined behavior.
  											Maybe somewhere in the process it's getting cleaned, but i could find it */
};
